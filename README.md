...
```
├── App/                    # 应用层
│   ├── aht20.h/.c         # AHT20 温湿度驱动
│   ├── ina226.h/.c        # INA226 电源监控驱动
│   ├── sd3078.h/.c        # SD3078 RTC 驱动
│   ├── icm42688.h/.c      # ICM-42688 IMU 驱动
│   ├── lcd.h/.c           # ST7789V2 LCD 驱动 + 字库
│   ├── lcd_page.h/.c      # 多页显示框架
│   ├── key.h/.c           # 按键消抖 + EC11 编码器
│   ├── bluetooth.h/.c     # 蓝牙驱动 + JSON 协议
│   ├── ws2812.h/.c        # WS2812 PWM+DMA 驱动
│   ├── sensor_data.h/.c   # 全局数据结构 + 任务间通信
│   └── tf_card.h/.c       # TF 卡 FATFS + CSV 记录
├── BSP/                   # 板级支持包
...
1. **拨码开关 BIT3 = OFF**（SPI2 模式），**BIT7 = OFF**（板载 RGB 模式）
2. 插入 TF 卡（格式化为 FAT32）
3. 上电——LCD 显示传感器数据页
